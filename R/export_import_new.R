      ...
      for (n_obser in n_vals) {
        for (i in 1:n_simul) {
          ...
          general_params_default <- list(
            ...
            keep_every = 1, num_chains = num_chains, verbose = F, 
            sample_global_prior = "half-cauchy", unlink = T, propensity_seperate = "none", gibbs = F, step_out = 0.5, max_steps = 150, save_output = F, probit_outcome_model = F, interaction_rule = "continuous_or_binary",standardize_cov = F, simple_prior = F, save_partial_residual = F, regularize_ATE = F,
