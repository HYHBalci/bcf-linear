...
library(DescTools)
library(grid)
# Or if you are using it for arranging multiple plots:
library(gridExtra)
...
# NEW HELPER: Compute Threshold Confidence Metrics
  ...
  for (het in heterogeneity_opts) {
    ...
    for (lin in linearity_opts) {
      for (n_obser in n_values) {
        ...
        # Expanded to 12 columns to include threshold metrics
        ...
        for (i in 1:n_simul) {
          ...
          tryCatch({
            ...
            data <- generate_data_medical(
              ...
              seed = i, RCT = FALSE, scenario = scen, z_diff = 0.5, BCF = FALSE, sigma_sq = 1
            )
        ...
        if(length(idx) > 0) {
          # Update columns 5 through 16 directly from the combined matrix
...
# 3. FINAL OUTPUTS
# --------------------------------------------------------------------------
save(results, file = 'E:/tout!/results_linked_analysis_ranking.RData')
...
# Subset just the standard metrics for the initial plot
