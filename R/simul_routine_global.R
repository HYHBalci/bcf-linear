    ...
    for(n_obser in n){
      for(i in 1:n_simul){
        ...
        general_params_default <- list(
          ...
          keep_every = 1, num_chains = 2, verbose = F, global_shrinkage = T, unlink = T
        )
        
        data <- generate_data_2(n_obser, is_te_hetero = het, is_mu_nonlinear = lin, seed = i, RCT = F)
        ...
        filename <- sprintf("BCF_GLOB_T_UNLINK_T_fit_heter_%s_linear_%s_n_%d_sim_%d.Rdata", 
                            ifelse(het, "T", "F"), 
                            ifelse(lin, "T", "F"), 
                            n_obser, 
                            i)
