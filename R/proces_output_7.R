...
for (n_obser in n_values) {
  for (het in heterogeneity_opts) {
    for (lin in linearity_opts) {
      ...
      for (i in 1:n_simul) {
        file_name <- sprintf(
          "C:/Users/P094412/Documents/PhD project/results/Block_linked/Block_linked_fit_heter_%s_linear_%s_n_%d_sim_%d.Rdata",
...
results_long <- all_results_df %>%
  ...
  pivot_longer(
    cols = c(ends_with("RMSE"), ends_with("Coverage"), ends_with("Length")),
    names_to = c("effect_type", "metric"),
    names_sep = "_",
    values_to = "value"
  ) %>%
  # Make metric names factors for consistent ordering in plots
  mutate(metric = factor(metric, levels = c("RMSE", "Coverage", "Length")))
...
################################################
library(coda)
load("C:/Users/P094412/Documents/PhD project/results/Block_linked/Block_linked_fit_heter_T_linear_T_n_500_sim_25.Rdata")
data <- generate_data_2(n = 500, is_te_hetero = TRUE, is_mu_nonlinear = TRUE, seed = 25)
