...
fit_grouped_horseshoes_R <- function(
  ...
  for (chain_num in 1:num_chains) {
    ...
    for (iter_idx in 1:n_iter) {
      ...
      post_mean_aleph <- (sum(w_vec * Z_vec * resid_for_aleph) / scale_factor + aleph_prior_mean * prior_prec_aleph) / post_prec_aleph
      ...
      if (p_hs_total > 0) {
        ...
        effective_global_taus_for_beta <- rep(1, p_hs_total)
        ...
        temp_lambda_scale_term <- (Beta_hs^2) / (2 * scale_factor * safe_var_R(effective_global_taus_for_beta^2))
        ...
        lambda_hs <- 1 / sqrt(safe_var_R(lambda_hs_sq_inv))
        ...
        tau_vector_components <- rep(1, p_hs_total); if(length(idx_prog) > 0) tau_vector_components[idx_prog] <- current_tau_prognostic
        if(length(idx_treat) > 0) tau_vector_components[idx_treat] <- current_tau_treatment
